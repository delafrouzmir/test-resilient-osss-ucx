...
int main(int argc, char const *argv[])
{
    ...
    FILE *fp;
    fp = fopen ("2rollback_matmul_res.txt", "a");
    ...
    *iter = 0;
    ...
    for ( (*iter)=0; (*iter)<num_iter; ++(*iter) )
    {
        ...
        if ( (*iter) == 50*frequency+5 && first_rollback == 1 ){
            first_rollback = 2;
            shmem_cpr_rollback(5, shmem_cpr_pe_num(me));
            // if ( cpr_pe_role == CPR_STORAGE_ROLE )
            *iter = 50*frequency;
            shmem_barrier_all();
        }
    ...
    shmem_barrier_all();
    if ( me == 0 )
        fprintf(fp, "2 rollback: npes=%d, spes=%d, array_size=%d, iter=%d, freq=%d\ntime=%f\n",
